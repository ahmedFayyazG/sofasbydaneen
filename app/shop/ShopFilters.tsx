...
const typeOptions: { key: ProductType; label: string }[] = [
  { key: "corner", label: "Corner sofas" }, { key: "chaise", label: "Chaise sofas" }, { key: "u-shape", label: "U-shape sofas" }, { key: "modular", label: "Modular sofas" }, { key: "chesterfield", label: "Chesterfield sofas" },
];
...
function ProductCard({ product, selectedColor }: { product: Product; selectedColor: ColorFamily | null }) {
  ...
  const monthly = Math.max(1, product.priceFromValue / 48).toFixed(2);
  ...
  return <article className="shop-product-card">
    <div className="shop-card-image" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      <span className="shop-delivery-badge">Delivery from 6 weeks</span>
    ...
    <div className="shop-card-info">
      ...
      <p className="shop-card-price"><strong>{product.priceFrom}</strong><span> | £{monthly} per month*</span></p>
...
export default function ShopFilters({ products, initialFilters }: { products: Product[]; initialFilters: RawFilters }) {
  ...
  const filteredProducts = useMemo(() => { const result = products.filter((product) => (!selectedType || product.type === selectedType) && (!selectedColor || product.colorFamilies.includes(selectedColor)) && (!selectedSize || matchesSize(product, selectedSize))); if (sort === "price-asc") return [...result].sort((a, b) => a.priceFromValue - b.priceFromValue); if (sort === "price-desc") return [...result].sort((a, b) => b.priceFromValue - a.priceFromValue); return result; }, [products, selectedType, selectedColor, selectedSize, sort]);
  ...
  return <div className="shop-page shop-collection-page">
    <div className="shop-toolbar">
      <button type="button" className="shop-filter-trigger" onClick={() => setFiltersOpen((open) => !open)}>Filter</button>
    ...
    {filtersOpen && <section className="shop-filter-panel" aria-label="Filter sofas"><div><h3>Type</h3>{typeOptions.map((option) => <button key={option.key} className={selectedType === option.key ? "active" : ""} onClick={() => setFilter("type", option.key)}>{option.label}</button>)}</div><div><h3>Colour</h3><div className="shop-filter-colours">{colorOptions.map((option) => <button key={option.key} className={selectedColor === option.key ? "active" : ""} onClick={() => setFilter("color", option.key)}><i style={{ background: option.hex }} />{option.label}</button>)}</div></div><div><h3>Size</h3>{sizeOptions.map((option) => <button key={option.key} className={selectedSize === option.key ? "active" : ""} onClick={() => setFilter("seats", option.key)}>{option.label}</button>)}</div>{hasFilters && <button className="shop-clear-link" type="button" onClick={clearFilters}>Clear all</button>}</section>}
    ...
    <div className="shop-count-line">Showing {filteredProducts.length} of {products.length} products</div>
    <section className="shop-results" aria-label="Sofa results">{filteredProducts.length ? <div className="shop-product-grid">{filteredProducts.map((product) => <ProductCard product={product} selectedColor={selectedColor} key={`${product.slug}-${selectedColor ?? "all"}`} />)}</div> : <div className="shop-empty"><h3>No sofas match these filters</h3><p>Try removing one filter to see more of the collection.</p><button type="button" onClick={clearFilters}>Clear filters</button></div>}</section>
  </div>;
}
