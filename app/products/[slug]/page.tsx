...
export default async function ProductPage({
  ...
  const trendingItems = products.map((item) => ({
    slug: item.slug,
    name: item.name,
    category: item.category,
    image: item.heroImage,
    price: item.priceFrom,
    fabric: item.fabrics.find((f) => f.code === item.defaultFabricCode)?.name ?? item.fabrics[0]?.name ?? "Made to order",
  }));
  ...
  return (
    <>
      ...
      <div id="top">
        ...
        <SiteHeader />
        <main id="main-content" className="dfs-product-page">
          <nav className="dfs-breadcrumb" aria-label="Breadcrumb">
        ...
        <footer id="showrooms" className="deferred-section">
          <Link className="footer-logo" href="/#top">Sofas By Daneen</Link>
          <div className="copyright">
            <span>LANGUAGE &nbsp; English</span>
            <span>Copyright © 2026 Sofas By Daneen. Designed by BureauBarbara</span>
          </div>
        </footer>
      </div>
