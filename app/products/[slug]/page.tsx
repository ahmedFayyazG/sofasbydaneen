...
import { getAllSlugs, getProduct, getProducts } from "../../lib/products";
...
import "./sofas-friends-layout.css";
import "./pdp-purchase-polish.css";
...
export default async function ProductPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ fabric?: string }> }) {
  ...
  const product = await getProduct(slug);
  if (!product) notFound();
  const structuredData = { "@context": "https://schema.org", "@type": "Product", name: product.name, description: product.tagline, image: product.heroImage, brand: { "@type": "Brand", name: "Sofas By Daneen" }, offers: { "@type": "AggregateOffer", priceCurrency: "GBP", lowPrice: product.priceFromValue, availability: product.inStock ? "https://schema.org/InStock" : "https://schema.org/OutOfStock", url: `${SITE_URL}/products/${product.slug}` } };
  const allProducts = await getProducts();
  const trendingItems = allProducts.map((item) => ({ slug: item.slug, name: item.name, category: item.category, image: item.heroImage, price: item.priceFrom, fabric: item.fabrics.find((f) => f.code === item.defaultFabricCode)?.name ?? item.fabrics[0]?.name ?? "Made to order" }));
