...
export default function ProductGallery({ product, initialFabricCode }: { product: Product; initialFabricCode?: string }) {
  ...
  const Options = ({ mobile = false, purchase = true }: { mobile?: boolean; purchase?: boolean }) => <div className={mobile ? "dfs-mobile-options" : ""}>
    ...
    {purchase && <><div className="dfs-price-area"><small>From</small><strong>{product.priceFrom}</strong><p>Flexible finance options available</p></div><div className="dfs-delivery-note">Handcrafted. Delivered to your home.</div><a className="dfs-primary-action" href="#showrooms">REQUEST A QUOTE</a><button className="dfs-secondary-action" type="button" onClick={() => setDrawerOpen(true)}>CHOOSE MATERIAL</button>{!mobile && <button className="dfs-shortlist" type="button">♡ &nbsp; Add to shortlist</button>}</>}
  </div>;
  ...
  return <>
    ...
    <section className="dfs-pdp-shell">
      <div className="dfs-gallery-column">
        ...
        <section className="dfs-mobile-purchase" aria-label="Price and purchase options"><div className="dfs-mobile-sale-price"><span>Sale Price</span><div><strong>{money(salePrice)}</strong><p><b>Save {money(saving)}</b><small>After Sale Price {money(previousPrice)}</small></p></div></div><div className="dfs-mobile-offer"><div><b>Summer Sale</b><span>ends in</span></div><div className="dfs-offer-countdown"><span><b>11</b><small>Days</small></span><span><b>07</b><small>Hours</small></span><span><b>04</b><small>Minutes</small></span><span><b>05</b><small>Seconds</small></span></div></div><div className="dfs-mobile-finance"><div className="dfs-finance-main"><span className="dfs-finance-icon">▣</span><div><p><b>{money(monthly)}</b> a month for 48 months</p><p>0% APR – No deposit</p><a href="#showrooms">Try our finance calculator</a></div></div><a className="dfs-finance-check" href="#showrooms">▱ &nbsp; Check your finance eligibility</a></div><div className="dfs-mobile-delivery">▰ &nbsp; Handcrafted. Delivered in est. 6 weeks</div><a className="dfs-mobile-basket" href="#showrooms">▱ &nbsp; Add to basket</a><div className="dfs-mobile-or"><span />or<span /></div><a className="dfs-mobile-phone" href="tel:+440000000000">⌕ &nbsp; Order by phone</a><button className="dfs-mobile-shortlist" type="button">♡ &nbsp; Add to shortlist</button></section>
        ...
        <div className="dfs-mobile-full-details"><section><h2>Description</h2>{product.intro.map((p, i) => <p key={i}>{p}</p>)}</section><section><h2>Key features</h2><ul><li>{product.silhouette}</li>{product.details.map((d) => <li key={d.label}><b>{d.label}:</b> {d.value}</li>)}</ul></section><section><h2>Dimensions &amp; configuration</h2><img src={angle} alt={`${product.name} configuration`} />{product.sizes?.map((s) => <p className="dfs-mobile-detail-row" key={s.seats}><b>{s.label}</b><span>{s.seats} seats</span></p>) ?? product.details.filter((d) => /configuration|width|depth|height/i.test(d.label)).map((d) => <p className="dfs-mobile-detail-row" key={d.label}><b>{d.label}</b><span>{d.value}</span></p>)}</section><section><h2>Delivery</h2><p>Your sofa is made to order. Access and delivery details are confirmed before dispatch.</p><p>{product.details.find((d) => d.label === "Lead time")?.value ?? "Delivery timing is confirmed when you place your order."}</p></section><section><h2>Reviews</h2><p>★★★★★ 4.9 average rating</p><p>Based on 32 reviews.</p></section></div>
      </div>
      <aside className="dfs-buy-column dfs-desktop-buy"><div className="dfs-desktop-summer-sale">SUMMER SALE</div><div className="dfs-product-heading"><h1>{product.name.replace(/^The\s+/i, "")}</h1><p>{product.tagline}</p><div className="dfs-stars">★★★★★ <span>4.9 · 32 reviews</span></div></div><Options /></aside>
    </section>
