...
export default function ProductGallery({ product, initialFabricCode }: { product: Product; initialFabricCode?: string }) {
  ...
  const chooseFabric = (code: string) => {
    if (!product.fabrics.some((f) => f.code === code)) return;
    setFabricCode(code);
    setGalleryIndex(0);
  };

  const chooseMaterial = (material: string) => {
    const first = product.fabrics.find((f) => getMaterial(f.name) === material);
    if (first) chooseFabric(first.code);
  };
  ...
  return <>
      ...
      <div className="dfs-gallery-column">
        ...
        <div className="dfs-gallery-tools">
          <div className="dfs-thumbs">
            {gallery.slice(0, 4).map((src, index) => <button type="button" key={`${src}-${index}`} className={galleryIndex === index ? "active" : ""} onClick={() => setGalleryIndex(index)}><img src={src} alt={`${product.name} view ${index + 1}`} /></button>)}
          </div>
    ...
    {drawerOpen && <div className="dfs-drawer-layer" role="dialog" aria-modal="true" aria-label="Choose your material">
      <button type="button" className="dfs-drawer-backdrop" aria-label="Close material selector" onClick={() => setDrawerOpen(false)} />
