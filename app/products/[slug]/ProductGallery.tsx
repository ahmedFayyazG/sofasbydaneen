...
export default function ProductGallery({ product, initialFabricCode }: { product: Product; initialFabricCode?: string }) {
  ...
  const SaleSummary = ({ mobile = false }: { mobile?: boolean }) => <section className={mobile ? "dfs-mobile-purchase dfs-sale-summary-only" : "dfs-desktop-purchase dfs-sale-summary-only"} aria-label="Sale price">
    ...
    <div className={mobile ? "dfs-mobile-offer" : "dfs-desktop-offer"}><div><b>Summer Sale</b><span>ends in</span></div><div className="dfs-offer-countdown"><span><b>11</b><small>Days</small></span><span><b>07</b><small>Hours</small></span><span><b>04</b><small>Minutes</small></span><span><b>05</b><small>Seconds</small></span></div></div>
  </section>;
  ...
  return <>
    ...
    <section className="dfs-pdp-shell">
      ...
      <aside className="dfs-buy-column dfs-desktop-buy"><div className="dfs-desktop-summer-sale">SUMMER SALE</div><div className="dfs-product-heading"><h1>{product.name.replace(/^The\s+/i, "")}</h1><p>{product.tagline}</p><div className="dfs-stars">★★★★★ <span>4.9 · 32 reviews</span></div></div><SaleSummary /><Options /><PurchaseActions /></aside>
    </section>
